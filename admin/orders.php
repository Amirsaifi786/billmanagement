<?php
    ...
    if(isset($_GET['orderid']) && isset($_GET['status']) && isset($_GET['user']))
    {
        ...
        $update = mysqli_query($conn,"UPDATE payment SET orderstatus='$status' WHERE orderid='$orderid'");
        if($update)
        {
            $selectold=mysqli_query($conn,"SELECT * FROM order status WHERE txn_id='$txn_id' AND userid='$userid' AND status='$status'");
           
             $selectstatus1 = mysqli_query($conn,"INSERT INTO `orderstatus`(`userid`, `txn_id`, `status`, `date`) VALUES ('$userid', '$txn_id', '$status', '$date')");
   ...
   <main role="main" class="main-content">
        <div class="container-fluid">
          <div class="row justify-content-center">
            <div class="col-12">
              <h2 class="mb-2 page-title" style="display:inline;">Manage Orders</h2>
                    ...
                    <div class="card-body">
                      ...
                      <table class="table datatables" id="dataTable-1">
                            ...
                            <tbody>
                                <?php
                                $i=1;
                                $selectdatatable=mysqli_query($conn,"SELECT * FROM payment WHERE paystatus='Success' GROUP BY txn_id ORDER BY date DESC ");
                            ...
                            <tr>
                                ...
                                <td><?php echo $date = $fetch['date']; ?></td>
				  <td>
            ...
            if($fetch['orderstatus']!='Delivered' && $fetch['orderstatus']!='Cancelled')
            {
               ...
               <?php if($fetch['orderstatus']=="Delivered"){ echo "Delivered"; } else { echo "Order status"; } ?>
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                  ...
                  {
                      ?>
                <a class="dropdown-item" href="orders.php?orderid=<?php echo $fetch['orderid']; ?>&status=Approve&user=<?php echo $fetch['userid']; ?>&txn_id=<?php echo $fetch['txn_id']; ?>">Approve</a>
                <?php   }
                  else if($fetch['orderstatus']=='Approve')
                  {
                      ?> 
                    <a class="dropdown-item" href="orders.php?orderid=<?php echo $fetch['orderid']; ?>&status=Shipped&user=<?php echo $fetch['userid']; ?>&txn_id=<?php echo $fetch['txn_id']; ?>">On the way</a>
                <?php
                  }
                  else if($fetch['orderstatus']=='Shipped')
                  {
                      ?>
                    <a href="orders.php?orderid=<?php echo $fetch['orderid']; ?>&status=Delivered&user=<?php echo $fetch['userid']; ?>&txn_id=<?php echo $fetch['txn_id']; ?> "class="dropdown-item">Delivered</a>
                <?php
                  }
                  ?> </div>
                </div>
